import { Box, Button, CloseButton, Divider, Input, InputGroup, InputRightElement } from '@chakra-ui/react';
...
export default function RecipientInput(props) {
  ...
  return (
    <Box border="1px" borderColor="gray.200" rounded="md" px="2" py="2" mb="2">
      <InputGroup size="md">
        <Input type="text" placeholder="0x0000000000000000000000000000000000000000" fontWeight="medium" isInvalid={inputAddr && !isAddress(inputAddr)}
          value={inputAddr} onChange={(e) => { onRecipientChange({ addr: e.target.value }); }} />
      ...
      <Box mt="2">
        <AdvancedNumberInputField decimals={decimals} maxAsBN={maxAsBN} showPercent  onChange={(v) => { onRecipientChange({ amountAsBN: v }); }} />
      </Box>
    </Box>
  );
}
