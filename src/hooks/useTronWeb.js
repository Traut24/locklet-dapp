...
import { TronConnectorEvent } from 'src/connectors/Tron/AbstractTronConnector';
...
export function createTronWebRoot(key) {
  ...
  return function TronWebProvider({ getLibrary, children }) {
    ...
    const library = useMemo(() => (active && chainId !== undefined && !!connector ? getLibrary(provider, connector) : undefined), [
      active,
      getLibrary,
      provider,
      connector,
